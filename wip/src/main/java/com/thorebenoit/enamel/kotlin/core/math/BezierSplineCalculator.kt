...
private object BezierSplineCalculator {
     ...
     */
    fun getCurveControlEPointTypesTriple(knots: List<EPoint>): List<Triple<EPoint?, EPoint?, EPoint>> {
        val ret = mutableListOf<Triple<EPoint?, EPoint?, EPoint>>()
        val n = knots.size - 1
        if (n < 1) {
            return ret
        }
        ...
        for (i in 0 until n) {
            ...
            // First control point
            val first = EPoint(x[i], y[i])

            // Second control point
            val second = if (i < n - 1)
                EPoint(2 * knots[i + 1].x - x[i + 1], 2 * knots[i + 1].y - y[i + 1])
            else
                EPoint((knots[n].x + x[n - 1]) / 2, (knots[n].y + y[n - 1]) / 2)
